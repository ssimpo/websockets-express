...
function completed(){document.removeEventListener("DOMContentLoaded",completed);window.removeEventListener("load",completed);onReady();}if(doc.readyState==="complete"||doc.readyState!=="loading"&&!doc.documentElement.doScroll){window.setTimeout(onReady);}else{document.addEventListener("DOMContentLoaded",completed);window.addEventListener("load",completed);}}/**
	 ...
	 */function setDefaultEndPoint(){var _ref=[global.location.origin,global.document.querySelector("base[href]")],origin=_ref[0],baseElement=_ref[1];var url;if(baseElement){var base=(baseElement.getAttribute("href")||"").trim().replace(origin,"");if(base!=="")url="".concat(origin).concat(base);}else{url="".concat(origin,"/");}url=url.replace("https://","wss://").replace("http://","ws://");endpoints.set(defaultSocketId,url);}/**
	 ...
	 */function createAcknowledge(resolve,reject){return function(err,response){if(err)return reject(err);if((response.status||200)>=400){if(err)return reject(err);var message=(response.body||response.statusMessage||"").trim();var _status2=response.status||400;return reject(new HTTP_ERROR({message:message,status:_status2}));}return resolve(response);};}/**
	 ...
	 */function connecting(ws,url,socketId){/**
		 ...
		 */function open(){status.set(socketId,SOCKETSTATUS.CONNECTED);if(!!WebSocketService.DEBUG)console.log("Opened ".concat(url," for ").concat(socketId));ws.addEventListener("close",close);ws.addEventListener("message",message);runSendQueue(socketId);}/**
		 * Close listener, try to reconnect.
		 */function close(){status.set(socketId,SOCKETSTATUS.CLOSED);if(!!WebSocketService.DEBUG)console.log("Closed ".concat(url," for ").concat(socketId));ws.removeEventListener("open",open);ws.removeEventListener("close",message);ws.removeEventListener("message",message);reconnect(url,socketId,ws);}/**
	 ...
	 *
	 * @singleton
	 */var WebSocketService=/*#__PURE__*/function(){function WebSocketService(){_classCallCheck(this,WebSocketService);// Singleton pattern
if(!WebSocketServiceInstance)WebSocketServiceInstance=this;this.addParser("json",defaultJsonParser);return WebSocketServiceInstance;}/**
		 ...
		 *
		 * @returns {string}
		 */},{key:Symbol.toStringTag,get:function get(){return"WebSocketService";}}]);return WebSocketService;}();WebSocketService.DEBUG=false;init();})(window);
