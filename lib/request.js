...
function getRawHeaders(req, message) {
	return req.rawHeaders.concat(
		util.flatten(Object.keys(message.headers || []).map(headerName=>[headerName, headers[headerName]]))
	);
}
...
class WebsocketRequest extends EventEmitter {
	constructor(req, message) {
		...
		// @todo Better handling needed here so we can handle binary and normal data wit ease
		//util.defineProperty(this, 'body', getBody(message), true);
		util.defineProperty(this, 'body', message.body, true);
		...
		util.definePropertyFixed(this, 'rawHeaders', getRawHeaders(req, message));
